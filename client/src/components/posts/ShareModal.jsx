  ...
  return (
    <div className="fixed inset-0 z-40 grid place-items-center bg-slate-900/40 p-4">
      <form className="grid w-[min(560px,100%)] gap-3 rounded-lg border border-[#c8d7e6] bg-white/95 p-4 shadow-[0_14px_34px_rgba(43,101,151,0.12)]" onSubmit={handleSubmit}>
        ...
        <textarea className="min-h-[104px] w-full resize-y rounded-md border border-[#c8d7e6] bg-white px-3 py-3 text-[#152033] outline-none focus:border-[#1776da] focus:shadow-[0_0_0_4px_rgba(23,118,218,0.13)]" value={content} onChange={(event) => setContent(event.target.value)} placeholder="Thêm suy nghĩ của bạn..." />
        ...
        <div className="rounded-lg border border-[#dbe6f1] bg-[#f7fbff] p-3">
          ...
          <p className="mt-1">{post.content || "Bài viết đa phương tiện"}</p>
        </div>
